"""Model A — vol regime classifier.

4-class output: low / range / trend / high_vol_expansion. Implementation:
LightGBM multiclass with feature_sets.A_VOL_REGIME. Labels are derived in
training/labels.py from forward realized vol percentiles + ADX.
"""
...
class VolRegimeModel(BaseModel):
    name = "vol_regime_a"
    ...
    def fit(self, X: pl.DataFrame, y: np.ndarray,
        ...
        defaults = {"objective": "multiclass", "num_class": 4,
    ...
    @classmethod
    def load(cls, artifact: ModelArtifact) -> "VolRegimeModel":
